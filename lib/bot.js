...
async function canUserConfirmBranch(user, branchId) {
  ...
  // 1) Registered manager chat_id bound to this branch
  try {
    const { getDb } = require('./db');
    const db = getDb();
    ...
    // 2) user_access ACL: manager of this branch
  ...
  // 3) Static roster username for this branch
...
async function sendGroupMessage(text, options = {}) {
  ...
  try {
    // Prefer raw Telegram API so reply_markup is reliable
    const payload = {
      chat_id: gid,
      text,
    ...
    // Fallback to bot client without buttons
    try {
      const bot = getBot();
      if (bot) {
      ...
      console.error('Group send fallback failed:', e2.message);
    }
...
async function notifyGroupNewDelivery(deliveryId) {
  ...
  const text =
    ...
    `📏 Расстояние: ${Math.round(d.distance || 0)} м\n🕐 Время: ${d.created_at}\n` +
    `🆔 #${d.id}\n\n` +
    `🔐 Подтвердить может только: ${managerTag}\n` +
    `⏳ Ожидает подтверждения.`;

  await sendGroupMessage(text, { reply_markup: confirmKeyboard(deliveryId) });
}
...
async function processDeliveryConfirmation(
  ...
  // Edit the original message (group or private) and strip buttons
  let edited = false;
  if (messageIdToEdit && chatIdToEdit) {
    try {
      ...
      try {
        ...
        console.warn('[bot] editMessage failed:', e2.message || e.message);
      }
    }
  }

  // If we didn't edit a group message, post final status to group
  const gid = await getGroupChatId();
  const editedInGroup = edited && gid && Number(chatIdToEdit) === Number(gid);
  if (!editedInGroup) {
...
async function handleUpdate(body) {
  ...
  if (body.callback_query) {
    ...
    const deliveryId = Number(data.split('_')[1]);
    const user = query.from;
    if (!user?.id || !deliveryId) {
      await answerCallback(query.id, 'Ошибка данных');
      return;
    }
    ...
    if (delivery.status !== 'pending') {
      ...
      // strip buttons if still present
      try {
    ...
    if (!auth.ok) {
      await answerCallback(
        query.id,
        `⛔ Только ${auth.responsible} (${auth.branch_name}) может подтвердить`,
        true
      );
      return;
    }
    ...
    try {
      if (bot) {
        await bot.sendMessage({
          ...
          text,
          parse_mode: 'Markdown',
          reply_markup: JSON.stringify(confirmKeyboard(deliveryId)),
        });
      }
    } catch (e2) {
      console.error(`Manager notify failed (${branchId}):`, e2.message || e.message);
    }
  }
}
